...
- The Golden Gate stands above the fog at the lower left of the sky photograph, seen down its span so the near tower is large and the bridge recedes up the left edge, on the home page and the 404 page: `public/images/golden-gate.avif` (PNG fallback) is rendered from the owner's bridge model (`archive/models/golden-gate-bridge.usdz`) by `scripts/prepare-golden-gate.swift` (Model I/O export), `scripts/render-golden-gate.mjs` (three.js in headless Chromium, International Orange, the sky's light, plus a data pass of world height and camera distance per pixel) and `scripts/make-golden-gate-sprite.py` (per-pixel fog fade, haze thickening with distance). `.bay-landmark` anchors it to the photograph in image fractions: the poster is a size container, so the sprite follows the photo's cover scaling on every aspect ratio and drifts with it.
- An Immersive control in the journey's bottom row asks the browser for fullscreen (the Fullscreen API, with the webkit names iPhone Safari uses since 16.4); it is hidden where fullscreen is unavailable and in a Home Screen launch, which has no browser bars to hide.
- On phones the page is edge to edge: `viewport-fit=cover` with the controls kept inside the safe areas, the journey's sticky frame sized to the large viewport so the sky and the aircraft fill the screen once Safari's bars collapse (the chapter bar stays anchored to the small viewport), no overscroll bounce, and `site.webmanifest` plus the Apple web-app tags so Add to Home Screen opens the site without browser chrome behind a translucent status bar.
