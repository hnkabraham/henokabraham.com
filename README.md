...
- `app/dreamliner-scene.tsx`: transparent Three.js renderer, HDR illumination, self-shadows, turning fans, wing flex, adaptive resolution and deterministic resource cleanup. The lens is carried rather than bolted down: four slow drifts on frequencies that never line up wander the aim by about four tenths of a degree, the lens itself by a few centimetres, the roll by a seventh of one and the focal length by half a percent, which is roughly a percent of the frame and well inside the depth cut's own softness.
- `lib/dreamliner-tour.ts`: camera choreography and portrait framing. The original chapter IDs remain valid: `preflight` → Open sky, `roll` → Apps, `liftoff` → Devices, `bay` → Flight log, `cruise` → Explore. The choreography is a fly-by from astern, a hold on the port engine's exhaust and a departure that the chapter stops sample at increasing distance. The departure holds a shallow left turn that shows the flexed wings from above and behind, and the aim falls behind it in moves that are never given back: the swing that carries the tail across the Apps caption leaves the aircraft up in the top left, and from there it only climbs, out through the top on a landscape screen and the top left corner on a phone, gone by the end of the scroll. Nothing takes it back down or back across — which is also what keeps it clear of the Golden Gate, standing near the middle of the portrait sky, that it once seemed set to meet. The tour check asserts it never falls back down the frame after the crossing and that it leaves by the top.
- `lib/dreamliner-cut.ts`: the aircraft through the words, twice. A caption hangs at a depth in front of the lens (`cutDepth` from the tour sampler) and sits under the renderer's canvas; the module rasterizes the caption word by word, in each word's own font, spacing and position, into a mask, and `addDepthCut` in `lib/airframe-flex.ts` has every opaque surface of the aircraft blend itself out where a glyph covers a fragment beyond that depth, so what is nearer covers the letters and what is beyond passes behind them. In the opening the plane is a fixed distance out and the wing sweeps through the headline. In the Apps chapter it rides out to the tail, which crosses the caption as the aircraft turns away, and sits at the lens on either side of that, where every letter is in front of the aircraft and the caption reads as it always has. A word, not a letter, is the unit: split finer and a ligature collapses a span to zero width, and the canvas cannot reproduce the page's letter-spacing per glyph. The tour check asserts both crossings on every viewport.
