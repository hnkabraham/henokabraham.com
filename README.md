...
My personal site: **[henokabraham.com](https://henokabraham.com)**. It opens with an airborne Boeing 787-9. The page begins with a moving blue sky; scrolling brings the aircraft past from behind the viewer into a look up one engine's tailpipe (its wing sweeping the headline as it goes, the letters jostled in its wake), then lets it fly on and away, wings flexed, banking gently into the distance. The flight reveals project previews and my flight log along the way, then leads into the full project board, route atlas, open-source work and contact form.
...
- `lib/dreamliner-tour.ts`: camera choreography and portrait framing. The original chapter IDs remain valid: `preflight` → Open sky, `roll` → Apps, `liftoff` → Devices, `bay` → Flight log, `cruise` → Explore. The choreography is a fly-by from astern, a hold on the port engine's exhaust and a departure that the chapter stops sample at increasing distance.
- `lib/dreamliner-wake.ts`: the wing's wake over the story text. The renderer projects the wing's planform (read off the model) to the screen each frame, clipped to the near plane and the frame, and the headline's letters and the caption's words nearest its edges are shoved along with its motion and spring back, buffeted, as it passes; the tour check asserts the visible wing reaches the headline on every viewport.
