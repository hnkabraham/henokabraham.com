...
- The Golden Gate's towers stand above the fog on the horizon of the sky photograph, on the home page and the 404 page: `public/images/golden-gate.avif` (14 KB, PNG fallback) is rendered from the owner's bridge model (`archive/models/golden-gate-bridge.usdz`) by `scripts/prepare-golden-gate.swift` (Model I/O export), `scripts/render-golden-gate.mjs` (three.js in headless Chromium, International Orange, the sky's light) and `scripts/make-golden-gate-sprite.py` (fog fade, haze tint). `.bay-landmark` anchors it to the photograph in image fractions: the poster is a size container, so the sprite follows the photo's cover scaling on every aspect ratio and drifts with it.
