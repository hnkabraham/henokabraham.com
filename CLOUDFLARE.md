...
Source `7c21ddd` replaces the ground departure with a moving-sky fly-in and engine, wing and tail close-ups. The credited FlightGear exterior has 4K fuselage/engine textures and 105,622 triangles merged into ten main material groups; it uses HDR reflections and a personal livery. The original terrain experiment remains in the repository and is absent from active scene imports. Source `b63af95` changes the HTTP Early Hint from the old tile manifest to the universal sky image. The aircraft remains a low-priority, motion-qualified HTML preload.
...
Source `b1918b3` adds a compact airline-logo strip below the country flags. The 14 airlines are sorted by aggregate flight count within the selected year or complete log. Selecting a logo shows the airline name and total; changing the year clears the selection. The public summary adds only carrier names, local logo paths and counts, with individual flights still excluded.
